...
function readRecord(db, lodash, response) {
    ...
    db.then(connection => {
        var result = connection.query("SELECT * FROM person ORDER BY personId DESC");
...
function searchRecord(db, lodash, request,response) {
    // as usual read we didn't need transaction .. 
    console.log(" test "+request.body.search);
    db.then(connection => {
        var result = connection.query("select * from person where name like CONCAT( '%',?,'%') OR age like CONCAT( '%',?,'%');", [request.body.search,request.body.search]);
        // we don't know  why mariad db send meta value soo 
        // so now we remove those whateer weird key 
        lodash.difference(result['meta']);
        return result;
...
export default { createRecord, readRecord, searchRecord,updateRecord, deleteRecord };